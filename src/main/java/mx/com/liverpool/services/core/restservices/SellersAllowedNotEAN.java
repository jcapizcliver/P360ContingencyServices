...
/**
 * Servlet implementation class SellersAllowedNotEAN
 */
@WebServlet("/public/rt/SellersAllowedNotEAN")
public class SellersAllowedNotEAN extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SellersAllowedNotEAN() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		qp.put("lookup", "'MarketplaceExcepProvEAN'");
		org.json.JSONArray rows = null;
		org.json.JSONArray values = null;
		
		jsonResponse = workshop.makeRequest("GET", "/list/LookupValue/bySearch", qp, null);
		if(jsonResponse != null) {
			rows = jsonResponse.getJSONArray("rows");
			for(int i=0; i<rows.length(); i++) {
				values = rows.getJSONObject(i).getJSONArray("values");
				objects.put(values.getString(0));
			}
		}else {
