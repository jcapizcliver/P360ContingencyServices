 ...
 */
@WebServlet("/public/rt/ws_list_valid_values_for_parent_template_characteristic_enabler_")
public class GetListOfValuesEnabler_ extends HttpServlet {
     ...
     */
    public GetListOfValuesEnabler_() {
        super();
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	try(DBAccessDataStub dastub = new DBAccessDataStub(new ELog() {
    		...
    		public void logE(Exception e) {
    			GetListOfValuesEnabler_.this.logE(e);
    		}
    		...
    		r = dr.getTemplateCharacteristicMetaDataByTemplateCharacteristicProperty(
    				new org.json.JSONArray().put(
    						new org.json.JSONObject()
    								.put("template", template)
    								.put("characteristic", characteristic)
    								.put("property", "dependentValues")
    				)
    		);
    		...
    		try {
    			...
    			validValues =
    					item.has(characteristic)
    					&& item.getJSONObject(characteristic).has("dependentValues")
    							? item.getJSONObject(characteristic).getString("dependentValues")
    							: null;
    		...
    		if(validValues != null) {
    			...
    			r = dr.getTemplateCharacteristicMetaDataByTemplateCharacteristicProperty(
    					new org.json.JSONArray().put(
    							new org.json.JSONObject()
    									.put("template", template)
    									.put("characteristic", characteristic)
    									.put("property", "dependentAttribute")
    			...
    			try {
    				...
    				dependentAttribute =
    						item.has(characteristic)
    						&& item.getJSONObject(characteristic).has("dependentAttribute")
    								? item.getJSONObject(characteristic).getString("dependentAttribute")
    								: null;
    			...
    			if(dependentAttribute != null) {
    				...
    				try {
    					...
    					lookup =
    							!"".equals(item.getString("lookup"))
    									? item.getString("lookup")
    									: null;
    				...
    				if(validValues != null && lookup != null) {
    					...
    					qp.put(
    							"fields",
    							"LookupValue.Code,LookupValueLang.Name(es),LookupValueIdentifier.Code(ATG)"
    					);
    					qp.put(
    							"query",
    							"LookupValue.IsActive equals true and LookupValue.Code in ("
    							+ sb0.toString()
    							+ ")"
    					);
    					qp.put("pageSize", "900");
    					...
    					do {
    						...
    						resp = workshop.makeRequest(
    								"GET",
    								"/list/LookupValue/bySearch",
    								qp,
    								null
    						);
    						...
    						for(int i = 0; i < rows.length(); i++) {
    							...
    							lookupValues.put(
    									new org.json.JSONObject()
    											.put("code", values.getString(0))
    											.put("name", values.getString(1))
    											.put("altCode", values.getString(2))
    							);
    						}
    			...
    			try {
    				...
    				org.json.JSONObject jr = new org.json.JSONObject(r);
    				org.json.JSONArray items = jr.getJSONArray("items");
    				java.util.Map<String, java.util.Map<String, String>> characteristics =
    				...
    				for(int i = 0; i < items.length() && !found; i++) {
    					...
    					for(String name : org.json.JSONObject.getNames(item)) {
    						...
    						if(properties.containsKey("dependentValues")
    								&& name.equals(characteristic)) {
    			...
    			if(validValues != null) {
    				...
    				try {
    					...
    					for(int i = 0; i < items.length() && !found; i++) {
    						...
    						for(String name : org.json.JSONObject.getNames(item)) {
    							...
    							if(properties.containsKey("dependentAttribute")
    									&& name.equals(characteristic)) {
    				...
    				if(dependentAttribute != null) {
    					...
    					r = dr.getCharacteristicData(
    							new org.json.JSONArray().put(dependentAttribute)
    					);
    	...
    	boolean alternatives =
    			Boolean.parseBoolean(
    					includeAlternative != null
    							? includeAlternative.toLowerCase()
    							: "false"
    			);
    		...
    		vls.put(
    				alternatives
    						? new org.json.JSONObject()
    								.put(
    										"label",
    										lookupValues.getJSONObject(i).getString("name")
    								)
    								.put(
    										"hex",
    										lookupValues.getJSONObject(i).getString("altCode")
    								)
    						: lookupValues.getJSONObject(i).getString("name")
    		);
    	}
